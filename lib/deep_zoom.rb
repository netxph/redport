...
class DeepZoom
  ...
  def initialize(image_path, options = {})
    ...
    if File.file? image_path
      ...
      orig_dir = File.dirname(@image_path)
    elsif is_valid_url? image_path
    ...
    @name = options[:name] ? options[:name] : orig_filename
    @extension = options[:format] ? options[:format] : orig_extension
    @quality = options[:quality]

    # set pathes to files and directories
    root_dir = options[:dir] ? options[:dir] : orig_dir
    @levels_root_dir     = File.join(root_dir, @name + '_files')
    @xml_descriptor_path = File.join(root_dir, @name + '.dzi')
  end
